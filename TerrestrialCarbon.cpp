...
double TerrestrialCarbon::CalculateEquilibriumLeafMass(EnvironmentCell* LocalEnvironment, bool deciduous ) {
    ...
    double TotalAET = LocalEnvironment->TotalAET();
    ...
    double FracEvergreen = CalculateFracEvergreen( LocalEnvironment->FractionYearFrost());

    // Calculate the fire mortality rate
    double FireMortRate = CalculateFireMortalityRate( NPP, LocalEnvironment->FractionYearFire());
...
double TerrestrialCarbon::UpdateLeafStock( EnvironmentCell* LocalEnvironment, Stock* z, bool deciduous ) {
    ...
    NPP *= LocalEnvironment->Seasonality();
    ...
    if( deciduous ) {
        ...
        TimeStepLeafMortRate = MonthlyLeafMortRate * TimeStep::instance()->MonthsPerTimeStep();
    ...
    // Calculate fine root mortality rate
    double AnnualFRootMort = CalculateFineRootMortalityRate( LocalEnvironment->Temperature() );
    ...
    // Calculate the structural mortality rate
    double StMort = CalculateStructuralMortality( LocalEnvironment->AET() * 12 );
    ...
    WetMatterIncrement *= TimeStep::instance()->MonthsPerTimeStep();
    ...
    double NPPWetMatter = std::max( -z->_TotalBiomass, WetMatterIncrement );
